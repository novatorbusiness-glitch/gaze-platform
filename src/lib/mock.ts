...
export interface Lesson {
  ...
  /** Модуль/этап курса, к которому относится урок (для курса «До 200к») */
  moduleId?: string
}

/** Модуль/этап курса «До 200к»: месяц пути с темой и уроками */
export interface CourseModule {
  id: string
  /** Название этапа, напр. «Средний чек» */
  title: string
  /** Что даёт этап (подзаголовок месяца) */
  subtitle?: string
  /** Эмодзи-иконка этапа */
  emoji?: string
}
...
export interface Course {
  ...
  readers?: number
  created_at: string
  /** Модули/этапы курса (для курса «До 200к» — 6 месяцев). Если заданы — уроки группируются по модулям. */
  modules?: CourseModule[]
  /** Оффер-подзаголовок обложки (напр. «50–60к → 200к за 6 месяцев») */
  offer?: string
}
